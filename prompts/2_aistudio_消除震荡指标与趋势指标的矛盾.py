    ...
    【高级交易逻辑修正】
    1. **识别“指标钝化”**：
    - 如果 RSI > 70 且 MACD 柱状图持续变长（动能增强），这不代表要卖出，反而代表“超强趋势”，此时应 **BUY** 或 **HOLD**，而不是 SELL。
    - 只有当 RSI > 80 且出现背离（价格新高但RSI没新高），或者 MACD 柱状图开始缩短时，才考虑 SELL。
    2. **指标权重动态调整**：
    - 当价格突破布林带上轨且不回落时（单边行情），**以 MACD 为主**，忽略布林带压力。
    - 当布林带走平（震荡行情）时，**以 RSI 和 布林带为主**，高抛低吸。
